//
//  Config+Loading.swift
//  Config file loading: reads ~/.config/umber/config.json and resolves it
//  field-by-field into an AppConfig, falling back to defaults on every error.
//
//  Split from Config.swift when that file exceeded the 350-line ceiling (#57).
//  The seam is clean: `load()` is the only caller of the `ConfigFile`-typed
//  private structs, and the font-resolution helpers it uses are self-contained
//  pure functions. `ConfigFile` was `private` in Config.swift; widening to
//  `internal` keeps it module-private (same effective scope in a single-module
//  package) and is required so this file can reference the type.
//

import AppKit

// MARK: - Font resolution helpers
