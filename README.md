...
copy/paste/select-all · window position restored across launches · **measured umber
palette by default**. Installing a theme is safe for the 256-colour cube: Umber pins
the xterm ANSI strategy before installing anything, so a theme moves indices 0–15
only and the standard cube survives (see `app/README.md`). `classic-repaired` keeps
Terminal.app Basic's true black, quiet body text and saturated ring while repairing
its unreadable blue; `classic`, `afk-dark`, `afk-light` and `tokyo-night` remain one
config line away.
